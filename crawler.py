...
def crawl(link, level):
	url_at_this_level = []
	print("Level: {}".format(level))
	response = urllib.request.urlopen(url)
	html = response.read()
	soup = BeautifulSoup(html, "html.parser")
	for sublinks in soup.find_all('a'):
		href = sublinks.get('href')
		if href is not None:
			if not href.find(link): # the sub url here is a relative url need to concatenate it
				href += link + href
				url_at_this_level.append(href)

def crawling(link, level):
	print("Level: {}".format(level))
	response = urllib.request.urlopen('http://www.cnn.com/')
	html = response.read()
	soup = BeautifulSoup(html, "html.parser")
